...
export default class extends Controller {
  ...
  loadIngredients() {
    if (this.ingredientsValue.length > 0) {
      this.ingredientsValue.forEach((ingredient) =>
        this.addIngredientWithValue(ingredient),
      );
    } else {
      this.addIngredient();
    }
    this.updateAddIngredientButtonState();
  }

  loadInstructions() {
    if (this.instructionsValue.length > 0) {
      this.instructionsValue.forEach((instruction) =>
        this.addInstructionWithValue(instruction),
      );
    } else {
      this.addInstruction();
    }
  ...
  addIngredientWithValue(value, focus = false) {
    ...
    if (focus) {
      ...
      const lastInput =
        this.ingredientsListTarget.lastElementChild.querySelector("input");
      lastInput.focus();
    }
    this.updateAddIngredientButtonState();
  }
  ...
  updateAddIngredientButtonState() {
    const lastInput =
      this.ingredientsListTarget.lastElementChild.querySelector("input");
  ...
  updateInstructionNumbers() {
    this.instructionsListTarget
      .querySelectorAll("[data-step-number]")
      .forEach((el, index) => {
